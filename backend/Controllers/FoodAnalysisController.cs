...
namespace FoodCalorie.Controllers
{
    ...
    public class FoodAnalysisController : ControllerBase
    {
        ...
        public async Task<IActionResult> Analyze([FromForm] FoodAnalysisRequest request)
        {
            ...
            // 4) Build AI payload for JSON-only output with portion size in grams
            var payload = new
            {
                model = "o4-mini",
                messages = new object[]
                {
                    new {
                        role = "system",
                        content = @"You are a food vision specialist. 
Analyze the food image and return a JSON array of objects, each with:
  - count: عدد العناصر
  - name: اسم العنصر
  - portionSizeGrams: حجم الحصة الواحدة بالجرامات (عدد)
...
Please respond **only** with valid JSON. لا تضف أي تفسير آخر."
        ...
        public class FoodItem
        {
            ...
            [JsonPropertyName("portionSizeGrams")]  public float  PortionSizeGrams   { get; set; }
